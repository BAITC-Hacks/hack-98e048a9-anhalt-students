...
- Динамическая загрузка исторических SCADA-данных (`POST /api/upload/scada` и кнопка в веб-интерфейсе): валидация схемы колонок, проверка физических диапазонов `[0, 1]`, сохранение и потокобезопасное переобучение моделей турбин с присвоением статуса `USER_SUPPLIED_VERIFIED`.
...
6. Загрузка пользовательского SCADA-файла через кнопку «Загрузить SCADA» в веб-интерфейсе или через API:
   ...
   ```
   Сервер проверяет столбцы и диапазоны, сохраняет файл в `data/shelek_historical.csv`, переобучает LightGBM модели турбин и возвращает обновлённые метрики валидации и статус `USER_SUPPLIED_VERIFIED`.
...
Автоматическая проверка (47 unit-тестов):
